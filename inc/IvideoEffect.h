...
#define     POST_CHROMA			   0x00004000   //色调				  Y			Y	14	
	...
	{
		//设置后台截图模式，在这个模式下，视频播放无显示。
		// 如果后面接的是null render自动开启此模式
	...
	DECLARE_INTERFACE_(IPostVideoEffectEx, IPostVideoEffect)
	{
		STDMETHOD(Scale)(THIS_ int width,int height) PURE;
		STDMETHOD(Clip)(THIS_ int l,int r,int t,int b) PURE;
		STDMETHOD(GetScaleParam)(THIS_ int *width,int *height) PURE;
		STDMETHOD(GetClipParam)(THIS_ int *l,int *r,int *t,int *b) PURE;
	};
